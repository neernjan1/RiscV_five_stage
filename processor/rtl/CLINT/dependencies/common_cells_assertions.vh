...
`ifndef CLINT_COMMON_CELLS_ASSERTIONS_SVH
`define CLINT_COMMON_CELLS_ASSERTIONS_SVH
...
`ifdef PRIM_ASSERT_SV
  `ifdef INC_ASSERT
    ...
    `undef ASSUME
    `undef ASSUME_I
    // Unlike the block above, these three are undefined unconditionally
    // (not just under FPV_ON): both this file's and prim_assert.sv's
    // versions are `ifdef FPV_ON no-ops when FPV_ON isn't set, so there's
    // no behavioral difference either way -- but leaving the redefinition
    // in place still produces a REDEFMACRO warning for no benefit.
    `undef ASSUME_FPV
    `undef ASSUME_I_FPV
    `undef COVER_FPV
  `endif
...
`endif // CLINT_COMMON_CELLS_ASSERTIONS_SVH