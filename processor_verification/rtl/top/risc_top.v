...
apb_top apb_interface (
    ...
    // .u
    .uart_PRDATA(uart_PRDATA), // Connect to UART peripheral
    .uart_PREADY(uart_PREADY), // Connect to UART peripheral
...
//uart 
 //wire PSLVERR,INT,   OUT1N, OUT2N, RTSN,    DTRN, CTSN, DSRN, DCDN, RIN, SIN, SOUT;
 // UART status/control signals
wire PSLVERR, INT;
...
apb_uart uart(
    .CLK (clk),
    .RSTN   (~rst),          // UART expects active-low reset

    .PSEL   (uart_psel),
    .PENABLE(penable),
    .PWRITE (pwrite),

    .PADDR  (paddr[4:2]),    // IMPORTANT: UART addr is small
    .PWDATA (pwdata),
